...
class TestSpending:
    ...
    def test_spending_page_elements(self, spending_page, logged_in_user):
        ...
        with allure.step("Проверка что находимся на странице добавления трат"):
            assert spending_page.is_loaded(), f"Не на странице трат. URL: {spending_page.page.url}"
    ...
    def test_valid_spending_creation(self, spending_page, logged_in_user):
        ...
        spending_actions = SpendingActions(spending_page.page)
        spending_data = SpendingBuilder().with_amount(1500).with_random_currency().with_category(
            "Food").with_description("Lunch at restaurant").build()
    ...
    def test_navigation_to_spending(self, main_page, logged_in_user):
        ...
        spending_actions = SpendingActions(main_page.page)
