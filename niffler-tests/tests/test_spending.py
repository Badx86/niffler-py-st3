...
class TestSpending:
    ...
    def test_spending_page_elements(self, page, logged_in_user):
        ...
        with allure.step("Проверка что находимся на странице добавления трат"):
            assert spending_page.is_loaded(), f"Не на странице трат. URL: {page.url}"
        ...
        with allure.step("Проверка основных элементов формы"):
            ...
            assert spending_page.CURRENCY_DROPDOWN.is_visible(), "Dropdown Currency не виден"
            ...
            assert spending_page.DESCRIPTION_INPUT.is_visible(), "Поле Description не видно"
    ...
    def test_currency_dropdown(self, page, logged_in_user):
    ...
    def test_date_picker(self, page, logged_in_user):
        ...
        with allure.step("Проверка что календарь отображается"):
            ...
            assert calendar_month.is_visible(), f"Заголовок месяца {spending_page.get_current_month_year()} не виден"

    @allure.story("Проверка значений по умолчанию")
    def test_default_values(self, page, logged_in_user):
        ...
        with allure.step("Проверка дефолтных значений"):
            ...
            assert actual_date == expected_date, f"Дефолтная дата неверная. Ожидали: {expected_date}, получили: {actual_date}"

            assert spending_page.get_description_placeholder() == "Type something", "Неверный placeholder"

    @allure.story("Создание валидной траты")
    def test_valid_spending_creation(self, page, logged_in_user):
        ...
        with allure.step("Проверка возврата на главную страницу"):
            ...
            assert "/main" in page.url, "Не вернулись на главную страницу"

    @allure.story("Кнопка отмены")
    def test_cancel_button(self, page, logged_in_user):
        ...
        with allure.step("Проверка возврата на главную"):
            page.wait_for_url("**/main", timeout=5000)
            assert "/main" in page.url, "Cancel не вернул на главную страницу"

    @allure.story("Валидация обязательных полей")
    def test_required_fields(self, page, logged_in_user):
        ...
        with allure.step("Проверка сообщений об ошибках"):
            assert spending_page.AMOUNT_ERROR.is_visible(), "Нет ошибки валидации для Amount"
            assert spending_page.CATEGORY_ERROR.is_visible(), "Нет ошибки валидации для Category"
        ...
        with allure.step("Проверка что ошибка Amount исчезла, но Category осталась"):
            assert not spending_page.AMOUNT_ERROR.is_visible(), "Ошибка Amount не исчезла после исправления"
            assert spending_page.CATEGORY_ERROR.is_visible(), "Ошибка Category должна остаться"
        ...
        with allure.step("Проверка успешного сохранения"):
            page.wait_for_url("**/main", timeout=5000)
            assert "/main" in page.url, "Не перешли на главную после исправления всех ошибок"

    @allure.story("Валидация суммы")
    def test_amount_validation(self, page, logged_in_user):
        ...
        with allure.step("Ввод различных значений суммы"):
            ...
            assert "999.99" in spending_page.get_amount_value(), "Не принимает дробные числа"

            spending_page.fill_amount(1000000)
            assert "1000000" in spending_page.get_amount_value(), "Не принимает большие числа"

    @allure.story("Ввод категории")
    def test_category_input(self, page, logged_in_user):
        ...
        with allure.step("Ввод различных категорий"):
            ...
            for category in test_categories:
                ...
                assert spending_page.CATEGORY_INPUT.input_value() == category, f"Категория {category} не сохранилась"

    @allure.story("Навигация на страницу трат")
    def test_navigation_to_spending(self, page, logged_in_user):
        ...
        with allure.step("Проверка перехода на страницу трат"):
            page.wait_for_url("**/spending", timeout=5000)
            spending_page = SpendingPage(page)
