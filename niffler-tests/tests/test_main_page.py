...
class TestMainPage:
    ...
    def test_main_page_elements(self, page, logged_in_user):
        ...
        with allure.step("Проверка что находимся на главной странице"):
            assert main_page.is_loaded(), f"Не на главной странице. URL: {page.url}"

        with allure.step("Проверка основных элементов"):
            assert main_page.LOGO.is_visible(), "Логотип Niffler не видим"
            assert main_page.NEW_SPENDING_BUTTON.is_visible(), "Кнопка New spending не видима"
            assert main_page.PROFILE_BUTTON.is_visible(), "Кнопка профиля не видима"

        with allure.step("Проверка заголовков разделов"):
            assert main_page.STATISTICS_TITLE.is_visible(), "Заголовок Statistics не виден"
            assert main_page.HISTORY_TITLE.is_visible(), "Заголовок History of Spendings не виден"
    ...
    def test_time_filter_dropdown(self, page, logged_in_user):
        ...
        with allure.step("Проверка наличия всех опций фильтра времени"):
            ...
            assert main_page.TIME_OPTION_LAST_MONTH.is_visible(), "Опция 'Last month' не видна"
            assert main_page.TIME_OPTION_LAST_WEEK.is_visible(), "Опция 'Last week' не видна"
    ...
    def test_profile_menu(self, page, logged_in_user):
        ...
        with allure.step("Проверка наличия всех пунктов меню профиля"):
            assert main_page.PROFILE_MENU_PROFILE.is_visible(), "Пункт 'Profile' не виден"
            assert main_page.PROFILE_MENU_FRIENDS.is_visible(), "Пункт 'Friends' не виден"
            assert main_page.PROFILE_MENU_ALL_PEOPLE.is_visible(), "Пункт 'All People' не виден"
            assert main_page.PROFILE_MENU_SIGN_OUT.is_visible(), "Пункт 'Sign out' не виден"
    ...
    def test_empty_state(self, page, logged_in_user):
        ...
        with allure.step("Проверка сообщения об отсутствии трат"):
            ...
            assert main_page.NO_SPENDINGS_MESSAGE.is_visible(), "Сообщение 'There are no spendings' не видимо"
